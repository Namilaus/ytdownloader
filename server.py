...
def give_data(download_method: str):
    ...
    if download_method == "download_video":
        try:
            data = (
                request.json
            )  # searching for download url in database for faster respond
            result = database.serach_for_video(data["url"])
            if (
                len(result) == 0 or result == None
            ):  # if its not in database then we scrap it and savte to database for future use
                dw_url = download_video(data["url"])
                database.insert_video(data["url"], dw_url)
                return {"title": dw_url["titles"], "url": dw_url["urls"]}
            else:
                return {"title": result[0][3], "url": result[0][2]}
        ...
        try:
            ...
            if len(results) == 0 or results == None:
                ...
                return {"titles": dw_url["titles"], "urls": dw_url["urls"]}
            else:
